...
class Board {
  ...
  private getDimensions(): IDimensions {
    ...
    const padTop: number = (this.height - canvasHeight) / 2
    const padLeft: number = (this.width - canvasWidth) / 2
    ...
    return {
      ...
      padTop,
      padLeft,
      padRight: padLeft + canvasWidth,
      padBottom: padTop + canvasHeight,
    }
  }
  ...
  public draw(): void {
    ...
    const image = <CanvasImageSource> document.getElementById('tile')

    let xEnd: number = left + this.dimensions.width
    let yEnd: number = top + this.dimensions.height
